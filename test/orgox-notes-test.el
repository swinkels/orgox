...
(ert-deftest export-to-ox-hugo-buffer ()
  ;; The ox-hugo buffer that `orgox-notes-export-to-ox-hugo-buffer' populates
  ;; has the same text as the note buffer and include the front matter for the
  ;; Hugo site and the Org properties for the headline of the note.
  (with-tmp-dir
   tmp-dir
...
(defun create-config (&rest kwargs)
  (pcase-let (((map :hugo-base-dir :markdown-file-name :notes-url) kwargs))
    (list :hugo-base-dir (if hugo-base-dir hugo-base-dir "/path/to/Hugo/site/repo")
          :markdown-file-name (if markdown-file-name markdown-file-name "20260101.md")
          :notes-url (if notes-url notes-url  "https://notes/repo/tree/main"))))
