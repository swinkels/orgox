...
#+begin_src emacs-lisp
(use-package orgox
  ;; When you've added `orgox' to `dotspacemacs-additional-packages' as a local
  ;; package, the Orgox autoloads won't have been loaded. In that case you need
  ;; to specify them explicitly in a :commands section.
  :commands
  (orgox-export-current-buffer orgox-publish-current-buffer)
  :custom
...
~orgox-export-current-buffer~ (or ~orgox-publish-current-buffer~) for the first
time:
- load Orgox;
